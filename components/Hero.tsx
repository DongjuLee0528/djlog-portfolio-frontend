...
const Hero: React.FC = memo(() => {
  ...
  // 개별 아이템 애니메이션 설정 - 메모이제이션으로 불필요한 재계산 방지
  ...
  // 3D 모델 상태에 따른 애니메이션 값 메모이제이션
  const modelAnimationValues = useMemo(() => ({
    opacity: isModelLoaded && !showFallback ? 1 : 0,
    scale: isModelLoaded && !showFallback ? 1 : 0.9
  }), [isModelLoaded, showFallback]);

  return (
    <section
      className="relative w-full min-h-screen flex flex-col md:flex-row items-center justify-center pt-20 md:pt-0 overflow-hidden bg-[#F7F7F7]"
      aria-label="메인 히어로 섹션"
    >
      
      {/* 텍스트 콘텐츠 영역 */}
      <header className="w-full md:w-1/2 px-6 md:pl-20 lg:pl-32 z-10 flex flex-col justify-center h-full order-2 md:order-1 mt-6 md:mt-0 pb-16 md:pb-0">
        <motion.div
        ...
        >
          ...
          <motion.h1
            ...
            className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-[#222222] leading-[1.1] mb-6 tracking-tight"
            tabIndex={0}
          >
            Solving Problems <br />
          ...
          <motion.p
            variants={itemVariants}
            className="text-base md:text-xl text-[#333333]/70 max-w-lg mb-8 leading-relaxed"
            role="banner"
      ...
      <div
        className="w-full md:w-1/2 h-[45vh] md:h-screen relative order-1 md:order-2 flex items-center justify-center"
        role="img"
        aria-label="3D 로봇 캐릭터 모델"
      >
        {/* 로딩 상태 UI */}
        {!isModelLoaded && !showFallback && (
          <motion.div 
          ...
          >
            <Loader2 className="w-10 h-10 text-[#4A90E2] animate-spin mb-4" aria-hidden="true" />
            <span className="text-sm font-medium text-[#333333]/50 animate-pulse" aria-live="polite">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center text-center p-8">
               <div className="w-20 h-20 rounded-2xl bg-white shadow-lg flex items-center justify-center mb-4 rotate-12 border border-gray-100">
                  <AlertCircle className="w-10 h-10 text-[#4A90E2]" aria-hidden="true" />
               </div>
               <h3 className="text-lg font-bold text-[#222222] mb-2" role="alert">3D View Unavailable</h3>
               <p className="text-sm text-[#333333]/60 max-w-xs leading-relaxed" aria-describedby="fallback-description">
        ...
        >
          ...
          <iframe
            ...
            title="3D 로봇 캐릭터 인터랙티브 모델 - 마우스로 회전하고 확대/축소 가능"
            ...
            tabIndex={0}
          />
          <span id="model-description" className="absolute left-[-10000px] w-[1px] h-[1px] overflow-hidden">
            3D 로봇 캐릭터 모델입니다. 마우스 또는 터치로 회전, 확대/축소가 가능합니다.
            키보드 사용자는 Tab 키로 포커스하여 상호작용할 수 있습니다.
          </span>
        </motion.div>
        
        {/* 자연스러운 통합을 위한 그라디언트 오버레이 */}
        <div
          className="absolute inset-0 pointer-events-none z-20 bg-gradient-to-b from-transparent via-transparent to-[#F7F7F7] md:bg-gradient-to-l md:to-transparent md:from-transparent md:via-transparent"
          aria-hidden="true"
        />
      </div>
